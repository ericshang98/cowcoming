...
状态：网站、云端消息服务与本机示例均已实现；实际设备驱动和真实 JEV/LLM 由接入者提供。后续进化修订已加入默认每 5 轮完整对话的独立 LLM 评估，规则及网关合同见 [进化运行合同](evolution-runtime.md)；未配置网关时不自动进化。本文补充产品文档 §3.3、§5，以 Eric 在当前任务中提出的“密钥接入、朋友独立开发、提示词影响 JEV、视频本地优先”为准。

## 给接入者的最新修正

先读 [进化与六种动作交接](hardware-handoff.md)。Eric 最新确认的六种回应为确认点头、摇头、郑重批准、得意双点头、左侧好奇歪头、右侧好奇歪头；不含转头看向。下文记录的是仍在运行的旧协议，动作白名单尚未迁移，不能把目标产品定义当作线上已支持的 ID。新增进化规则与已实现/待配置部分也在交接中区分。
...
- 左侧显示云端配置的形态、提示词版本和电脑端应用状态。点击左侧节点现在直接选择逻辑形态并进入手动模式，通过版本校验同步云端配置；同步完成前保留设备实际形态与确认状态。
...
进化计轮兼容字段：`language.start` 可携带所属交互的 `commandId`，更新后的 Python 套件已发送此字段。浏览器只计入同版本、非模拟、文本与动作都已完成的关联交互。旧套件缺少关联字段时仍可查看文本，但不会误计轮。
