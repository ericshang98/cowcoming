...
- 六套模型保留五项已核验骨骼动作，并在软件层提供桌面宠物公共动作目录的程序化扩展：头部与视线、躯干、前肢、腹部、尾巴/姿态和组合表达。左右以角色自身为准；`WAIT` 为待机控制。
...
共享动作逻辑只用于牛来进化形态，其他 IP 的展示逻辑独立。桌面宠物公共动作目录按身体区域组织；网页软件层会按形态和事件叠加已登记的表现变体，BenBen 仍只接收自己声明的五个兼容动作 ID。新增模型变体必须补充资产、骨骼和浏览器验证，不能把软件变体当成已验证的机械臂能力。
