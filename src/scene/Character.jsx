...
function Rig({ human, modelAsset = MASCOT, characterId, controller, placement, visible, onReady, entrance, onTap, shadowRef }) {
  ...
  useEffect(() => {
    ...
    const player=createResponsePlayer({mixer,actions,manifest:responseManifest,bones:names,getSoftwareVariant:(request,_variant,context)=>selectSoftwareVariant(responseManifest.formId,request.actionId,request.eventId,context),getTuning:(id)=>(controller.motionTuning || loadTuning(browserTuningStorage()).document).forms[responseManifest.formId]?.actions[id] || DEFAULT_TUNING,onStart:()=>{
