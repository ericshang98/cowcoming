...
export function useEvolutionDeviceSync(session, live) {
  ...
  useEffect(() => {
    if (session.state.roomId !== live.snapshot?.roomId) { memory.current = null; return; }
    const plan = planEvolutionSync(memory.current, session.state, live.snapshot, live.online, live.error);
  ...
  }, [session.state.sessionId, session.state.generation, session.state.form, live.snapshot?.roomId, live.snapshot?.profile.revision, live.status, live.online, live.error, live.updateProfile, session.dispatch]);
