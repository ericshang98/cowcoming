...
export default function Evolution({ preview, onSelectRoute, onSelectForm, modelStatus }) {
  ...
  return (
    ...
    <section className="work-page evolution-page page" data-pwc-critical="work">
      ...
      <aside className="career evolution-timeline" aria-label="Evolution paths">
        ...
        {evolutionRoutes.some(item => item.nodes.slice(2).some(id => isFormRevealed(id))) && <div className="evolution-routes" role="group" aria-label="Switch evolution paths">
          {evolutionRoutes.map((item, index) => (
            <button key={item.id} aria-pressed={route.id === item.id} onClick={() => onSelectRoute(item.id)}>{isFormRevealed(item.nodes.at(-1)) ? item.name : `Path ${index + 1}`}</button>
          ))}
        </div>}
        ...
        <ol>
          {route.nodes.map((id, index) => (
            <li key={id} className={form.id === id ? 'current' : ''}>
              <button className="evolution-node" disabled={!isFormRevealed(id)} aria-pressed={form.id === id} onClick={() => onSelectForm(id)}>
                <span>{String(index + 1).padStart(2, '0')} / {!isFormRevealed(id) ? 'UNKNOWN' : id === 'calf' ? 'ORIGIN' : 'NEXT FORM'}</span>
                <strong>{isFormRevealed(id) ? forms[id].name : 'Undiscovered'}</strong>
                <small>{!isFormRevealed(id) ? 'Not yet revealed' : forms[id].model ? 'Preview form' : 'Model pending'}</small>
              </button>
            </li>
          ))}
        </ol>
      ...
      <ObservationPanel />
      <footer className="evolution-footer"><Controls music={false} onExpand={() => setTreeOpen(true)} /><span>Preview only · Hardware state is unchanged</span></footer>
    </section>
    {treeOpen && <EvolutionTree selected={form} onSelect={onSelectForm} onClose={() => setTreeOpen(false)} />}
    </></Localized>
  );
}
