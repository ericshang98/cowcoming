...
See [the complete hardware connection and API guide](docs/live-device.md). WORK requires a valid browser-key binding and an online local device before evolution or live panels are available. It supports a keyed device room, editable form prompts, JEV action-to-animation mapping, direct WebRTC video/CV and local-process LLM streams. Manual selection, reset and configurable evaluation cadence are implemented; automatic evolution needs a configured evaluation model and gateway. Read the [hardware developer handoff](docs/hardware-handoff.md) for the corrected five-action product contract and the remaining differences in the current protocol. The Python kit is generated into `public/downloads` by `npm run dev` / `npm run build`. Run `npm run test:relay` for an isolated Worker integration check.
