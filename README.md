...
**连接设备后可用：**手动选择六种逻辑形态、调整进化周期、重置为小牛，以及密钥房间内的设备事件、视频与文本流接口。自动进化默认每 5 轮完整对话评估一次，但必须接入真实会话并配置符合合同的评估网关；五种形态已接入独立模型和五段回应动作，骚牛模型及真实硬件驱动仍待补齐。预设示例不计入进化轮数。
...
- [最新动作交接](docs/hardware-handoff.md)：五种基础回应、动作合同 2、版本迁移和朋友接入实机的验收边界。
