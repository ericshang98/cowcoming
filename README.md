...
| 本地联调 GEV + 语言模型 | 运行 `npm run models:dev`，再运行网页开发服务；本地步骤见 [GEV 云端试玩](docs/gev-cloud-preview.md#本地联调) |
...
**本次分支交付：**六种形态已接入独立模型，并把动作扩展为按身体区域组织的桌面宠物行为目录。现有骨骼资源会为目录动作选择已登记的软件变体；真实硬件驱动仍待接入，设备只执行自己的 `supportedActions` 子集。具体版本和验收边界见团队交接入口，本段不代表正式网站已发布这些更新。连接设备后支持调整进化周期、重置为小牛，以及密钥房间内的设备事件、视频与文本流接口。自动进化默认每 5 轮完整对话评估一次，必须接入真实会话并配置符合合同的评估网关；预设示例不计入进化轮数。
...
- [最新动作交接](docs/hardware-handoff.md)：桌面宠物行为目录、设备能力子集、版本迁移和朋友接入实机的验收边界。
- [桌面宠物动作目录](docs/action-catalog.md)：按头部、躯干、前肢、腹部、尾巴和组合表达组织的可扩展行为合同。

网页使用 `browserKey`，设备程序使用 `deviceKey`；两者由房间管理员分别提供。公开源码不包含房间密钥、模型密钥或生产管理权限。动作目录是桌面宠物的公共语义层；BenBen 的五个旧动作只是一个设备能力子集，其他机器人可以声明自己的 `supportedActions`。
