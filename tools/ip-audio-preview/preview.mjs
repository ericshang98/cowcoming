...
function stop(message = '已停止。可以重新试听。') {
  request++; playing = false; audio.pause(); audio.currentTime = 0; cancelAnimationFrame(raf); $('status').textContent = message; draw();
}
...
async function play(input) {
  ...
  audio.src = item.src;
  audio.onended = () => { if (token !== request) return; playing = false; cancelAnimationFrame(raf); $('status').textContent = '播放完成。再按一次，或换个角色听听。'; draw(); };
...
try {
  ...
  $('tap').disabled = false; $('signature').disabled = false; select(manifest.characters[0].id);
